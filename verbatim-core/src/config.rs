...
pub struct PostProcessingConfig {
    ...
    pub prompt: String,
    #[serde(default = "default_pp_llm_model")]
...
pub struct HandsFreeConfig {
    #[serde(default)]
    pub enabled: bool,
    #[serde(default = "default_handsfree_hotkeys")]
    pub hotkeys: Vec<String>,
}
...
fn default_handsfree_hotkeys() -> Vec<String> { vec![] }
...
fn default_pp_llm_model() -> String { "llama-3.2-1b".into() }
...
impl Default for PostProcessingConfig {
    fn default() -> Self {
        Self {
            ...
            llm_model: default_pp_llm_model(),
...
impl Config {
    ...
    pub fn load_from(path: &std::path::Path) -> Result<Self> {
        ...
        if config.google.credentials_path.is_empty() {
            if let Ok(path) = std::env::var("GOOGLE_APPLICATION_CREDENTIALS") {
                tracing::debug!("overriding google.credentials_path from GOOGLE_APPLICATION_CREDENTIALS env var");
                config.google.credentials_path = path;
            }
        }
...
mod tests {
    ...
    fn test_default_hotkeys_macos() {
        let config = Config::default();
        assert_eq!(config.general.hotkeys, vec!["KEY_F5"]);
    }
...
"#;
    ...
    fn test_config_default_hotkeys_linux() {
        let config = Config::default();
        assert_eq!(config.general.hotkeys, vec!["KEY_RIGHTCTRL"]);
    }
