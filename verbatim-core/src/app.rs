...
async fn process_and_output(ctx: &ProcessContext<'_>) -> Result<AppState> {
    ...
    match result {
        ...
        Ok(text) => {
            ...
            if ctx.backend_name == "openai" || ctx.backend_name == "deepgram" {
                let provider = if ctx.backend_name == "deepgram" { "deepgram" } else { "openai-stt" };
            ...
            let text = if ctx.backend_name == "deepgram" {
                ...
                tracing::debug!("starting post-processing");
                let pp = Arc::clone(pp);
                let raw = text.clone();
            ...
            if ctx.clipboard_only {
                ...
                if let Err(e) = clipboard::copy_to_clipboard(&text) {
                    ...
                    match EnigoBackend::new(ctx.input_method, ctx.paste_command, ctx.paste_rules) {
                        Ok(mut input) => {
...
impl SttService {
    ...
    pub async fn run(mut self) -> Result<()> {
        ...
        // Initialize the llama.cpp backend singleton BEFORE whisper loads.
        // Both whisper-rs-sys and llama-cpp-sys-2 share ggml symbols due to
        // --allow-multiple-definition; initializing llama first avoids a
        // double-init segfault.
        if let Err(e) = crate::llm_inference::init_backend() {
        ...
        let mut llm_engine: Option<Arc<LlmEngine>> = None;
        let mut post_processor: Option<Arc<PostProcessor>> = if self.config.post_processing.enabled {
            match self.create_post_processor(&mut llm_engine).await {
                Ok(pp) => {
                    if let Some(ref engine) = llm_engine {
                        if engine.gpu_fallback() {
                            let _ = self.gui_tx.send(SttEvent::GpuFallback(
                                "LLM model loaded in CPU-only mode (GPU unavailable)".into(),
                            ));
                        }
                    }
                    Some(Arc::new(pp))
        ...
        // Parse hotkeys and create shared configs for persistent listeners
        #[cfg(target_os = "linux")]
        let ptt_config = {
            let combos = evdev_listener::parse_hotkeys(&self.config.general.hotkeys)
                .with_context(|| format!("Invalid hotkeys: {:?}", self.config.general.hotkeys))?;
        ...
        let ptt_config = {
            ...
            for hk in &self.config.general.hotkeys {
                combos.push(macos_listener::parse_hotkey(hk)
        ...
        #[cfg(target_os = "linux")]
        let _hotkey_handle = evdev_listener::start_listener(ptt_config.clone(), hotkey_tx)
            .context("Failed to start hotkey listener")?;

        #[cfg(target_os = "macos")]
        let _hotkey_handle = macos_listener::start_listener(ptt_config.clone(), hotkey_tx)
            .context("Failed to start hotkey listener")?;
        ...
        let handsfree_config = {
            let combos = if handsfree_enabled && !current_handsfree_hotkeys.is_empty() {
                evdev_listener::parse_hotkeys(&current_handsfree_hotkeys).unwrap_or_else(|e| {
                    tracing::error!("Invalid hands-free hotkeys '{:?}': {}", current_handsfree_hotkeys, e);
                    Vec::new()
                })
            } else {
                Vec::new()
        ...
        let handsfree_config = {
            let combos = if handsfree_enabled && !current_handsfree_hotkeys.is_empty() {
                ...
                let mut ok = true;
                for hk in &current_handsfree_hotkeys {
                    match macos_listener::parse_hotkey(hk) {
                        Ok(combo) => parsed.push(combo),
                        Err(e) => {
                            tracing::error!("Invalid hands-free hotkey '{}': {}", hk, e);
                            ok = false;
                            break;
                        }
                    }
                }
                if ok { parsed } else { Vec::new() }
            } else {
                Vec::new()
            };
            macos_listener::SharedHotkeyConfig::new(combos)
        ...
        #[cfg(target_os = "linux")]
        let _handsfree_handle = evdev_listener::start_listener(handsfree_config.clone(), handsfree_tx)
            .context("Failed to start hands-free listener")?;

        #[cfg(target_os = "macos")]
        let _handsfree_handle = macos_listener::start_listener(handsfree_config.clone(), handsfree_tx)
            .context("Failed to start hands-free listener")?;
                    ...
                    match cmd {
                        ...
                        SttCommand::UpdateConfig(new_config) => {
                            ...
                            if new_config.general.hotkeys != current_hotkeys {
                                #[cfg(target_os = "linux")]
                                match evdev_listener::parse_hotkeys(&new_config.general.hotkeys) {
                                    Ok(new_combos) => {
                                        ptt_config.update(new_combos);
                                        current_hotkeys = new_config.general.hotkeys.clone();
                                        tracing::info!("Hotkeys updated to: {:?}", current_hotkeys);
                                    }
                                    Err(e) => {
                                        tracing::error!("Invalid hotkeys '{:?}': {}, keeping old hotkeys", new_config.general.hotkeys, e);
                                    }
                                }
                                ...
                                #[cfg(target_os = "macos")]
                                {
                                    ...
                                    for hk in &new_config.general.hotkeys {
                                        match macos_listener::parse_hotkey(hk) {
                                            Ok(combo) => new_combos.push(combo),
                                            Err(e) => {
                                                tracing::error!("Invalid hotkey '{}': {}, keeping old hotkeys", hk, e);
                                                parse_ok = false;
                                                break;
                                            }
                                        }
                            ...
                            if new_config.hands_free.hotkeys != current_handsfree_hotkeys {
                                ...
                                if handsfree_enabled && !current_handsfree_hotkeys.is_empty() {
                                    #[cfg(target_os = "linux")]
                                    match evdev_listener::parse_hotkeys(&current_handsfree_hotkeys) {
                                        Ok(new_combos) => {
                                            handsfree_config.update(new_combos);
                                            tracing::info!("Hands-free hotkeys updated to: {:?}", current_handsfree_hotkeys);
                                        }
                                        Err(e) => {
                                            tracing::error!("Invalid hands-free hotkeys '{:?}': {}", current_handsfree_hotkeys, e);
                                    ...
                                    #[cfg(target_os = "macos")]
                                    {
                                        ...
                                        if parse_ok {
                                            handsfree_config.update(combos);
                                            tracing::info!("Hands-free hotkeys updated to: {:?}", current_handsfree_hotkeys);
                                        } else {
                                            handsfree_config.update(Vec::new());
                                        }
    ...
    async fn create_post_processor(&self, llm_engine: &mut Option<Arc<LlmEngine>>) -> Result<PostProcessor> {
        ...
        match pp_config.provider.as_str() {
            ...
            "local" => {
                tracing::info!(
                    model = %pp_config.llm_model,
                    "Post-processing enabled (provider: local)"
                );
                // Reuse existing engine if same model, otherwise load new one
                let need_reload = match llm_engine.as_ref() {
                    None => {
                        tracing::debug!("No LLM engine loaded yet, need to load");
                        true
                    }
                    Some(eng) => {
                        let changed = eng.model_id() != pp_config.llm_model;
                        if changed {
                            tracing::info!(
                                old = eng.model_id(),
                                new = %pp_config.llm_model,
                                "LLM model changed, reloading"
                            );
                        } else {
                            tracing::debug!(model = eng.model_id(), "reusing existing LLM engine");
                        }
                        changed
                    }
                };
                if need_reload {
                    let model_dir = self.config.resolved_llm_model_dir();
                    if !llm_model_manager::llm_model_exists(&model_dir, &pp_config.llm_model) {
                        anyhow::bail!(
                            "LLM model '{}' not found. Please download it first.",
                            pp_config.llm_model
                        );
                    }
                    let model_path = llm_model_manager::llm_model_path(&model_dir, &pp_config.llm_model);
                    tracing::info!(
                        path = %model_path.display(),
                        model = %pp_config.llm_model,
                        "loading LLM model in background thread"
                    );
                    ...
                    let model_id = pp_config.llm_model.clone();
                    let engine = tokio::task::spawn_blocking(move || {
                        LlmEngine::load(&model_path, threads, &model_id)
                    })
                    .await
                    .map_err(|e| anyhow::anyhow!("LLM model load task failed (join error): {}", e))??;
                    tracing::info!(model = %engine.model_id(), "LLM engine ready");
                    *llm_engine = Some(Arc::new(engine));
                }
                Ok(PostProcessor::new_local(pp_config, llm_engine.as_ref().unwrap().clone()))
            }
            other => {
    ...
    async fn create_backend(&self) -> Result<Box<dyn SttBackend>> {
        ...
        match self.config.general.backend.as_str() {
            ...
            other => bail!(
                "Unknown backend: '{}'. Use 'whisper-local', 'openai', or 'deepgram'.",
                other
            ),
        }
    }
...
mod tests {
    ...
    async fn test_create_backend_unknown_errors() {
        ...
        let service = SttService::new(config, gui_tx, cmd_rx, None);
