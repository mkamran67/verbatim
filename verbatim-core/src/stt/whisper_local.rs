...
/// Set to true when whisper failed to init on GPU and fell back to CPU.
/// Surfaced to the UI once per process.
pub static GPU_FALLBACK: AtomicBool = AtomicBool::new(false);

pub fn gpu_fallback_occurred() -> bool {
    GPU_FALLBACK.load(std::sync::atomic::Ordering::Relaxed)
}
use whisper_rs::{
...
impl WhisperLocal {
    ...
    pub fn new(model_path: &Path, threads: u32) -> Result<Self, SttError> {
        ...
        let want_gpu = (cfg!(feature = "cuda") && crate::gpu_detect::detect_cuda_device())
            || cfg!(all(target_os = "macos", feature = "metal"));
        tracing::info!(use_gpu = want_gpu, "whisper GPU decision");

        let model_str = model_path.to_str().unwrap_or_default();
        let ctx = {
            let mut ctx_params = WhisperContextParameters::default();
            ctx_params.use_gpu(want_gpu);
            match WhisperContext::new_with_params(model_str, ctx_params) {
                Ok(c) => c,
                Err(e) if want_gpu => {
                    tracing::warn!(error = %e, "whisper GPU init failed, retrying on CPU");
                    GPU_FALLBACK.store(true, std::sync::atomic::Ordering::Relaxed);
                ...
                Err(e) => return Err(SttError::InferenceFailed(format!("Failed to load model: {}", e))),
            }
        };
