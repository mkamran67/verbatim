  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      ...
      {open && (
        <div
          ref={dropdownRef}
          className={`absolute z-50 mt-1 min-w-[200px] max-h-60 overflow-y-auto rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 shadow-lg py-1 ${
        ...
        >
          {options.map((opt) => (
            ...
            >
              <span>{opt.label}</span>
              {opt.hint && (
                <span className="ml-2 text-[10px] text-slate-400 dark:text-slate-500">{opt.hint}</span>
              )}
            </button>
          ))}
        </div>
